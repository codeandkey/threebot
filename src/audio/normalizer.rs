...
impl VolumeNormalizer {
    ...
    pub fn process(&mut self, samples: &mut [i16]) {
        ...
        if !self.loudness_buffer.is_empty() {
            let avg_loudness = self.loudness_buffer.iter().sum::<f32>() / self.loudness_buffer.len() as f32;
        ...
        if (self.current_gain - 1.0).abs() > 0.001 { // Only apply if gain is significantly different from 1.0
...
mod tests {
    ...
    fn test_db_to_linear_conversion() {
        ...
        assert!((result_6db - 2.0).abs() < 0.01, "6dB conversion failed: got {}, expected ~2.0", result_6db);
        assert!((result_neg6db - 0.5).abs() < 0.01, "-6dB conversion failed: got {}, expected ~0.5", result_neg6db);
    }
